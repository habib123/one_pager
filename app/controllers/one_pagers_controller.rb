class OnePagersController < ApplicationController
	before_action :mig_signed_in_user, only: [:edit, :update,:mig_message]
	#before_action :correct_user,   only: [:edit, :update]
       ...
       def one_pager_approve
    		#@one_pager = OnePager.find(params[:id])
    		@one_pager = OnePager.find(1)
       end
       ...
       def update
		@one_pager = OnePager.find(params[:id])
	  	@one_pager.comments = params[:one_pager][:comments]

	  if params[:commit] == 'Approve'
	    	@one_pager.mig_status = "Approve"
	  elsif params[:commit] == 'Deny'
	    	@one_pager.mig_status = "Deny"
	  end
	  ...
	  if  @one_pager.update_attributes(onepager_approve_params)
		...
		def onepager_approve_params
			params.require(:one_pager).permit(:commit,:comments)
		end
		def mig_signed_in_user
		    redirect_to mig_signin_path, notice: "Please sign in." unless mig_signed_in?
		end
